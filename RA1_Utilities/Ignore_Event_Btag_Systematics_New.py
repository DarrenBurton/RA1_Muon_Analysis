...
files = ["./Muon_Full_Dataset.root","./Had_Full_Dataset.root"]

#files = ["./Muon_Full_Dataset_NoTau.root","./Had_Full_Dataset.root"]

extra_files = ["./Muon_Multiplicity.root","./Had_Multiplicity.root"]

#files = ["./Muon_Ignore_Events.root","./Had_Ignore_Events.root"]


#files = ["./Muon_NoCuts.root","./Had_Full_Dataset.root"]


#files = ["./Muon_Test.root","./Had_Test.root"]

#files = ["./Muon_New_Attempt.root","./Had_New_Attempt.root"]


#files = ["./Muon_Full_Dataset_Nomatch.root","./Had_Full_Dataset_Nomatch.root"]
...
btag_holder = []
mistag_holder = []
muon_nomatch = dict.fromkeys(bins)
had_nomatch = dict.fromkeys(bins)
...
for label,path in enumerate(extra_files):
    ...
    for key in DirKeys:
      ...
      if dir == subdirect.GetName():
        for subkey in subdirect.GetListOfKeys():
          if subkey.GetName() == "Number_Matched_Multiplicity_all":
              plot = file.Get(dir+"/"+subkey.GetName())
              matched.append(plot)
          if subkey.GetName() == "Number_Not_Matched_Multiplicity_all":
              plot = file.Get(dir+"/"+subkey.GetName())
              not_matched.append(plot)
 
  for num,entry in enumerate(sorted(muon_nomatch.iterkeys())):
    for bin in range(1,16):      
        if label == 0:
          try: muon_nomatch[entry].append(1/(1-(not_matched[num].GetBinContent(bin)/matched[num].GetBinContent(bin))))
          except ZeroDivisionError :muon_nomatch[entry].append(1)
        else:
          if bin == 3 and num == 0: print not_matched[num].GetBinContent(bin)
          try: had_nomatch[entry].append(1/(1-(not_matched[num].GetBinContent(bin)/matched[num].GetBinContent(bin))))
          except ZeroDivisionError: had_nomatch[entry].append(1)


  for entry in sorted(muon_nomatch.iterkeys()):
    print muon_nomatch[entry]

  #print "Had"
  #for entry in sorted(had_nomatch.iterkeys()):
  #  print had_nomatch[entry]
...
predict_file = r.TFile("Had_Multiplicity.root")
predict_file_muon = r.TFile("Muon_Multiplicity.root")
#predict_file = r.TFile("Had_Ignore_Events.root")
#predict_file_muon = r.TFile("Muon_Ignore_Events.root")



#predict_file = r.TFile("Had_Full_Dataset.root")
#predict_file_muon = r.TFile("Muon_Full_Dataset.root")
...
for num,plot in enumerate(muon_list):
  ...
  for b in range(2,plot.GetNbinsX()):
    for i in range(1,plot.GetNbinsY()):
      print " b = %s, noB = %s" %(b,i)
      print "Altered = %f" %float(i*m*pow((1-m),i-1)*b*(b-1)*0.5*e*e*pow((1-e),b-2)*plot.GetBinContent(b+1,i+1)*muon_nomatch[str(num)][b+i])
      print "Normal = %f" %float(i*m*pow((1-m),i-1)*b*(b-1)*0.5*e*e*pow((1-e),b-2)*plot.GetBinContent(b+1,i+1))

      if b == 3: pred_three += i*m*pow((1-m),i-1)*b*(b-1)*0.5*e*e*pow((1-e),b-2)*plot.GetBinContent(b+1,i+1)*muon_nomatch[str(num)][b+i]
      else: pred_three += i*m*pow((1-m),i-1)*b*(b-1)*0.5*e*e*pow((1-e),b-2)*plot.GetBinContent(b+1,i+1)*muon_nomatch[str(num)][b+i]
  
  for b in range(3,plot.GetNbinsX()):
    for noB in range (1,plot.GetNbinsY()):
      if b == 3:pred_three += pow((1-m),noB)*b*(b-1)*(b-2)*e*e*e*pow((1-e),b-3)*(1/6)*plot.GetBinContent(b+1,noB+1)*muon_nomatch[str(num)][b+noB]
      else:pred_three += pow((1-m),noB)*b*(b-1)*(b-2)*e*e*e*pow((1-e),b-3)*(1/6)*plot.GetBinContent(b+1,noB+1)*muon_nomatch[str(num)][b+noB]
  
  for b in range (0,plot.GetNbinsX()):
    for noB in range(3,plot.GetNbinsY()):
      if b ==3:pred_three += pow((1-e),b)*noB*(noB-1)*(noB-2)*(1/6)*m*m*m*pow((1-m),noB-3)*plot.GetBinContent(b+1,noB+1)*muon_nomatch[str(num)][b+noB]
      else:pred_three += pow((1-e),b)*noB*(noB-1)*(noB-2)*(1/6)*m*m*m*pow((1-m),noB-3)*plot.GetBinContent(b+1,noB+1)*muon_nomatch[str(num)][b+noB]
  
  for b in range (1,plot.GetNbinsX()):
    for noB in range(2,plot.GetNbinsY()):
      if b == 3:pred_three += noB*e*pow((1-e),b-1)*noB*(noB-1)*0.5*m*m*pow((1-m),noB-2)*plot.GetBinContent(b+1,noB+1)*muon_nomatch[str(num)][b+noB]
      else:pred_three += noB*e*pow((1-e),b-1)*noB*(noB-1)*0.5*m*m*pow((1-m),noB-2)*plot.GetBinContent(b+1,noB+1)*muon_nomatch[str(num)][b+noB]
  ...
  for b in range (0,plot.GetNbinsX()):
    for noB in range(0,plot.GetNbinsY()):
      if b == 3: pred_zero += pow((1-e),b)*pow((1-m),noB)*plot.GetBinContent(b+1,noB+1)*muon_nomatch[str(num)][b+noB]
      else:pred_zero += pow((1-e),b)*pow((1-m),noB)*plot.GetBinContent(b+1,noB+1)*muon_nomatch[str(num)][b+noB]

  for b in range (0,plot.GetNbinsX()):
    for noB in range(1,plot.GetNbinsY()):
      if b == 3:pred_one += pow((1-e),b)*noB*m*pow((1-m),noB-1)*plot.GetBinContent(b+1,noB+1)*muon_nomatch[str(num)][b+noB]
      else:pred_one += pow((1-e),b)*noB*m*pow((1-m),noB-1)*plot.GetBinContent(b+1,noB+1)*muon_nomatch[str(num)][b+noB]
  
  for b in range(1,plot.GetNbinsX()):
    for noB in range(0,plot.GetNbinsY()):
      if b ==3:pred_one += pow((1-m),noB)*b*e*pow((1-e),b-1)*plot.GetBinContent(b+1,noB+1)*muon_nomatch[str(num)][b+noB]
      else:pred_one += pow((1-m),noB)*b*e*pow((1-e),b-1)*plot.GetBinContent(b+1,noB+1)*muon_nomatch[str(num)][b+noB]
  ...
  for b in range (0,plot.GetNbinsX()):
    for noB in range(2,plot.GetNbinsY()):
      if b == 3: pred_two += pow((1-e),b)*noB*0.5*(noB-1)*m*m*pow((1-m),noB-2)*plot.GetBinContent(b+1,noB+1)*muon_nomatch[str(num)][b+noB]
      else:pred_two += pow((1-e),b)*noB*0.5*(noB-1)*m*m*pow((1-m),noB-2)*plot.GetBinContent(b+1,noB+1)*muon_nomatch[str(num)][b+noB]
  
  for b in range (2,plot.GetNbinsX()):
    for noB in range(0,plot.GetNbinsY()):
      if b == 3:pred_two += pow((1-m),noB)*0.5*b*(b-1)*e*e*pow((1-e),b-2)*plot.GetBinContent(b+1,noB+1)*muon_nomatch[str(num)][b+noB]
      else: pred_two += pow((1-m),noB)*0.5*b*(b-1)*e*e*pow((1-e),b-2)*plot.GetBinContent(b+1,noB+1)*muon_nomatch[str(num)][b+noB]
  
  for b in range (1,plot.GetNbinsX()):
    for noB in range(1,plot.GetNbinsY()):
      if b ==3:pred_two += b*e*pow((1-e),b-1)*noB*m*pow((1-m),m-1)*plot.GetBinContent(b+1,noB+1)*muon_nomatch[str(num)][b+noB]
      else:pred_two += b*e*pow((1-e),b-1)*noB*m*pow((1-m),m-1)*plot.GetBinContent(b+1,noB+1)*muon_nomatch[str(num)][b+noB]
  ...
  diffb3_muon.Fill((pred_three - muon_btags[num].GetBinContent(4)) / muon_btags[num].GetBinContent(4));

  ratio = (pred_three - muon_btags[num].GetBinContent(4)) / muon_btags[num].GetBinContent(4)
  ...
  #c1.SaveAs("Muon_B_vs_noB_%s.png"%htbins[num])
l +="]\n"
s +="]\n"
...
for num,plot in enumerate(had_list):
  ...
  for b in range(2,plot.GetNbinsX()):
    for i in range(1,plot.GetNbinsY()):
      if b == 3: pred_three += i*m*pow((1-m),i-1)*b*(b-1)*0.5*e*e*pow((1-e),b-2)*plot.GetBinContent(b+1,i+1)*had_nomatch[str(num)][b+i]
      else: pred_three += i*m*pow((1-m),i-1)*b*(b-1)*0.5*e*e*pow((1-e),b-2)*plot.GetBinContent(b+1,i+1)*had_nomatch[str(num)][b+i]
  
  for b in range(3,plot.GetNbinsX()):
    for noB in range (1,plot.GetNbinsY()):
      if b == 3:pred_three += pow((1-m),noB)*b*(b-1)*(b-2)*e*e*e*pow((1-e),b-3)*(1/6)*plot.GetBinContent(b+1,noB+1)*had_nomatch[str(num)][b+noB]
      else:pred_three += pow((1-m),noB)*b*(b-1)*(b-2)*e*e*e*pow((1-e),b-3)*(1/6)*plot.GetBinContent(b+1,noB+1)*had_nomatch[str(num)][b+noB]
  
  for b in range (0,plot.GetNbinsX()):
    for noB in range(3,plot.GetNbinsY()):
      if b ==3:pred_three += pow((1-e),b)*noB*(noB-1)*(noB-2)*(1/6)*m*m*m*pow((1-m),noB-3)*plot.GetBinContent(b+1,noB+1)*had_nomatch[str(num)][b+noB]
      else:pred_three += pow((1-e),b)*noB*(noB-1)*(noB-2)*(1/6)*m*m*m*pow((1-m),noB-3)*plot.GetBinContent(b+1,noB+1)*had_nomatch[str(num)][b+noB]
  
  for b in range (1,plot.GetNbinsX()):
    for noB in range(2,plot.GetNbinsY()):
      if b == 3:pred_three += noB*e*pow((1-e),b-1)*noB*(noB-1)*0.5*m*m*pow((1-m),noB-2)*plot.GetBinContent(b+1,noB+1)*had_nomatch[str(num)][b+noB]
      else:pred_three += noB*e*pow((1-e),b-1)*noB*(noB-1)*0.5*m*m*pow((1-m),noB-2)*plot.GetBinContent(b+1,noB+1)*had_nomatch[str(num)][b+noB]
  
  error_three = (e*e*m)*sqrt(pow(plot.GetBinError(3,2),2 )+(4*(1-m)*(1-m)*pow(plot.GetBinError(3,3),2))+(9*pow((1-m),4)*pow(plot.GetBinError(3,4),2))+(16*pow((1-m),6)*pow(plot.GetBinError(3,5),2))+(25*pow((1-m),8)*pow(plot.GetBinError(3,6),2))     )
  
  for b in range (0,plot.GetNbinsX()):
    for noB in range(0,plot.GetNbinsY()):
      if b == 3: pred_zero += pow((1-e),b)*pow((1-m),noB)*plot.GetBinContent(b+1,noB+1)*had_nomatch[str(num)][b+noB]
      else:pred_zero += pow((1-e),b)*pow((1-m),noB)*plot.GetBinContent(b+1,noB+1)*had_nomatch[str(num)][b+noB]

  for b in range (0,plot.GetNbinsX()):
    for noB in range(1,plot.GetNbinsY()):
      if b == 3:pred_one += pow((1-e),b)*noB*m*pow((1-m),noB-1)*plot.GetBinContent(b+1,noB+1)*had_nomatch[str(num)][b+noB]
      else:pred_one += pow((1-e),b)*noB*m*pow((1-m),noB-1)*plot.GetBinContent(b+1,noB+1)*had_nomatch[str(num)][b+noB]
  
  for b in range(1,plot.GetNbinsX()):
    for noB in range(0,plot.GetNbinsY()):
      if b ==3:pred_one += pow((1-m),noB)*b*e*pow((1-e),b-1)*plot.GetBinContent(b+1,noB+1)*had_nomatch[str(num)][b+noB]
      else:pred_one += pow((1-m),noB)*b*e*pow((1-e),b-1)*plot.GetBinContent(b+1,noB+1)*had_nomatch[str(num)][b+noB]
  ...
  for b in range (0,plot.GetNbinsX()):
    for noB in range(2,plot.GetNbinsY()):
      if b == 3: pred_two += pow((1-e),b)*noB*0.5*(noB-1)*m*m*pow((1-m),noB-2)*plot.GetBinContent(b+1,noB+1)*had_nomatch[str(num)][b+noB]
      else:pred_two += pow((1-e),b)*noB*0.5*(noB-1)*m*m*pow((1-m),noB-2)*plot.GetBinContent(b+1,noB+1)*had_nomatch[str(num)][b+noB]
  
  for b in range (2,plot.GetNbinsX()):
    for noB in range(0,plot.GetNbinsY()):
      if b == 3:pred_two += pow((1-m),noB)*0.5*b*(b-1)*e*e*pow((1-e),b-2)*plot.GetBinContent(b+1,noB+1)*had_nomatch[str(num)][b+noB]
      else: pred_two += pow((1-m),noB)*0.5*b*(b-1)*e*e*pow((1-e),b-2)*plot.GetBinContent(b+1,noB+1)*had_nomatch[str(num)][b+noB]
  
  for b in range (1,plot.GetNbinsX()):
    for noB in range(1,plot.GetNbinsY()):
      if b ==3:pred_two += b*e*pow((1-e),b-1)*noB*m*pow((1-m),m-1)*plot.GetBinContent(b+1,noB+1)*had_nomatch[str(num)][b+noB]
      else:pred_two += b*e*pow((1-e),b-1)*noB*m*pow((1-m),m-1)*plot.GetBinContent(b+1,noB+1)*had_nomatch[str(num)][b+noB]
  ...
  #print "\n\n"
  #print had_btags[num].GetBinContent(4)
  try: diffb3.Fill((pred_three - had_btags[num].GetBinContent(4)) / had_btags[num].GetBinContent(4));
  except ZeroDivisionError: diffb3.Fill(0)
  try:ratio = (pred_three - had_btags[num].GetBinContent(4)) / had_btags[num].GetBinContent(4)
  except ZeroDivisionError: diffb3.Fill(0)
...
htbin = [275,325,375,475,575,675,775,875]
File = r.TFile("Btag_Two_Translation_Factors_plus.root","RECREATE") 
...
for box,entry in enumerate(array_keeper):
    for num,plot in enumerate(had_zero):
      ...
      histogram.SetBinContent(num+1,translation)
...
File_had = r.TFile("Btag_Two_Had_Change_plus.root","RECREATE") 
File_mu = r.TFile("Btag_Two_Mu_Change_plus.root","RECREATE") 
