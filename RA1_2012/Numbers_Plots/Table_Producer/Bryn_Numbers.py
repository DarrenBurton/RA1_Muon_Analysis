  ...
  def MakeList(self,dict,key,error = "",combined = ""):
      ...
      for entry in sorted(dict.iterkeys()):
        if error: 
          ...
          else: List.append(self.toString("%6.4f" %(dict[entry][key]+combined[entry][key] if combined else dict[entry][key]))+"  $\pm$  "+ self.toString("%6.4f" % (sqrt((dict[entry][error]*dict[entry][error])+(combined[entry][error]*combined[entry][error])) if combined else dict[entry][error])))
        else: 
          if dict[entry][key] == 0: List.append('-')
          else: List.append(self.toString("%6.4f" %(dict[entry][key]+combined[entry][key] if combined else dict[entry][key])))  
      return List  
