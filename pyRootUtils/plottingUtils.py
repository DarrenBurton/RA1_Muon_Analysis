...
class GetSumHist(object):
    ...
    for f in self.files:
      ...
      if self.norm is None or len(self.norm) is 1:
        for Dir in self.directories:
          if Dir is None:
              self.hObj = self.a.Get(self.hist)
              return self.hObj
          try: self.a.Get(Dir)
  ...
  def Integral(self,val1,val2):
    ...
    bin1 = val1#self.hObj.FindBin(val1)
    bin2 = val2#self.hObj.FindBin(val2)
    print "We are looking at the integral between  bin %i (low edge) %f and bin %i (low edge) %f, integral is %f"%(bin1, self.hObj.GetBinLowEdge(bin1), bin2,self.hObj.GetBinLowEdge(bin2),self.hObj.Integral(bin1,bin2))
